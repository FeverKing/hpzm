...
func NewScanner(ctx context.Context, options ...Option) (*Scanner, error) {
	...
	if scanner.binaryPath == "" && runtime.GOOS != "windows" {
		var err error
		scanner.binaryPath, err = exec.LookPath("nmap")
		if err != nil {
			return nil, ErrNmapNotInstalled
		}
	} else if scanner.binaryPath == "" && runtime.GOOS == "windows" {
		var err error
		scanner.binaryPath, err = GetNmapPathWindows()
		if err != nil {
			return nil, err
		}
...
func GetNmapPathWindows() (string, error) {
	nmapPath := "./lib/scan_lib/scan.exe"
	if _, err := exec.LookPath(nmapPath); err != nil {
		return "", errors.New("scanner not found in PATH")
	} else {
		return nmapPath, nil
	}
}
